...
const command = {
	...
	async execute(interaction: ChatInputCommandInteraction, _client: Client) {
		...
		const schoolData = JSON.parse(data) as {
		...
		for (const [key, roleData] of Object.entries(schoolData)) {
			...
			finalSchoolData.push({
				...
				percentage:
					roleData.studentCount > 0 ? (count / roleData.studentCount) * 100 : 0,
			});
		}

		finalSchoolData.sort((a, b) => b.percentage - a.percentage);
		// build a summary and detailed fields
		const totalStudents = finalSchoolData.reduce(
			(s, f) => s + f.studentCount,
			0,
		);
		const totalMembers = interaction.guild.members.cache.filter(
			(m) => !m.user.bot,
		).size;
		const overallParticipation =
			totalStudents > 0 ? (totalMembers / totalStudents) * 100 : 0;
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				`Servermedlemmar: **${totalMembers}** • Nationelltotal: **${totalStudents}** • Deltagande: **${Math.round(overallParticipation)}%**`,
			)
			.addFields(fields)
			.setColor(0x2b2d31)
			.setTimestamp()
