...
async function getAllSchoolData(): Promise<{ [key: string]: { id: string; studentCount: number } }> {
...
async function getSchoolData(school: Schools): Promise<number> {
    ...
    await axios
        ...
        .then((response: { data: { body: { totalNumberOfPupils: UnitsResponsePartial[]; }; }; }) => {
            const data = response.data.body?.totalNumberOfPupils[0] as UnitsResponsePartial;
